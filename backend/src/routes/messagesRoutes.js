...
router.get('/conversations', async (req, res) => {
  try {
    const currentUserId = req.user && req.user.userId;
    const rawData = await messageStore.listConversations(currentUserId);
    const conversations = Array.isArray(rawData) ? rawData : [];
    ...
    for (const convo of conversations) {
      ...
      metaByConvId.set(String(convo._id), {
        participants,
        otherId,
        pairKey,
      });
    ...
    for (const convo of conversations) {
      const meta = metaByConvId.get(String(convo._id));
      if (!meta) continue;
